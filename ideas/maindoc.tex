...
\subsubsection{One-point statistic of redshift and another parameter}
...
\node (mf) [hyper] {$\Phi(M,z)$};
\node (z) [param, below of=mf,yshift=-0.75cm,xshift=0.5cm] {$z_{j}$};
\node (M) [param, below of=mf,yshift=-0.75cm,xshift=-0.5cm] {$M_{j}$};
\node (flux) [data, below of=mf,yshift=-2cm] {$\vec{d}_{j}$};
\node (survey) [draw=black,fit={(M.west)(z.north)(flux.south)(z.east)}] {};
\node [xshift=1.75cm,yshift=0.25cm] at (survey.south) {$j=1,\dots,J$};
...
\subsubsection{One-point statistic of redshift and many other parameters}
...
\node (tf) [hyper,xshift=1cm] {$\theta(\vec{L},v,z)$};
...
\subsubsection{Two-point statistics of redshift and another parameter}
\subsubsection{Two-point statistics of redshift and many other parameters}
