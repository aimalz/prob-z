...
\node (mf) [hyper,xshift=-1cm] {$\Phi(M,z)$};
\node (nz) [hyper,xshift=0.5cm,yshift=-1cm] {$N(z)$};
\node (z) [param, below of=nz] {$z_{j}$};
\node (M) [param, below of=mf,yshift=-1cm] {$M_{j}$};
\node (flux) [data, below of=mf,yshift=-2cm,xshift=0.75cm] {$\vec{d}_{j}$};
