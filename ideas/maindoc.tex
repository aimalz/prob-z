...
We consider a second parameter $M_{j}$ that may represent the luminosity or mass of a galaxy $j$ in a survey.  We assume we are given a survey of joint posteriors $\{p(z_{j},M_{j}|\vec{d}_{j})\}_{J}$.  Fig. \ref{fig:mf} illustrates the conditionals of such a problem.
...
Let us consider the three-dimensional lensing potential $\textul{\psi}$ estimated from a survey of $N$ galaxy fluxes $\{\vec{f}_{n}\}_{N}$ and measured angular positions $\{\hat{\theta}_{n}\}_{N}$.  We assume the following quantities are known: $\{p(\vec{\theta}_{n}|\hat{\theta}_{n})\}_{N}$, $\{p(z_{n}|\vec{f}_{n})\}_{N}$, $\{p(\vec{x}_{n}|\textul{\psi})\}_{N}$, $p(\vec{x}_{n}|\vec{\theta}_{n},z_{n}\}_{N}$.
...
\node (t) [param, below of=pos,yshift=-0.5cm,xshift=-0.5cm] {$\vec{\theta}_{n}$};
\node (flux) [data, below of=z,yshift=-0.5cm] {$\vec{f}_{n}$};
\node (hatt) [data, below of=t,yshift=-0.5cm] {$\hat{\theta}_{n}$};
...
\begin{align*}
p(\textul{\psi}|\{\hat{\theta}_{n},\vec{f}_{n}\}_{N}) &= \frac{p(\textul{\psi})}{p(\{\hat{\theta}_{n},\vec{f}_{n}\}_{N})}p(\{\hat{\theta}_{n},\vec{f}_{n}\}_{N}|\textul{\psi})\\
&\propto p(\textul{\psi})p(\{\hat{\theta}_{n},\vec{f}_{n}\}_{N}|\textul{\psi})\\
&\propto p(\textul{\psi})\prod_{n=1}^{N}p(\hat{\theta}_{n},\vec{f}_{n}|\textul{\psi})\\
&\propto p(\textul{\psi})\prod_{n=1}^{N}\int p(\hat{\theta}_{n},\vec{f}_{n}|\vec{x}_{n})p(\vec{x}_{n}|\textul{\psi})d\vec{x}_{n}\\
&\propto p(\textul{\psi})\prod_{n=1}^{N}\int p(\vec{x}_{n}|\textul{\psi})\iint p(\hat{\theta}_{n}|\vec{\theta}_{n})p(\vec{f}_{n}|z_{n})p(\vec{\theta}_{n},z_{n}|\vec{x}_{n})d\vec{\theta}_{n}dz_{n}d\vec{x}_{n}\\
\end{align*}
%
%\begin{figure}
%\vspace{0.5cm}
