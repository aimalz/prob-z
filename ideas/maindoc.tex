...
\node (gl) [xshift=1.5cm,yshift=-0.25cm] at (g.south) {$i=1,\dots,I$};
\node (sl) [xshift=2.5cm,yshift=-0.5cm] at (s.south) {$j=1,\dots,J$};
\node (cl) [xshift=3cm,yshift=-0.75cm] at (c.south) {$j=1,\dots,K$};
...
\end{tikzpicture}
\caption{Here we wish to marginalize over tomographic binning strategies to estimate the cosmological parameters $\vec{\Omega}$.}
\label{fig:tomo}
\end{center}
\end{figure}
