...
%\item \citet{ben00} presents the BPZ template-based method of obtaining zPDFs and publishes code that produces the top three MAP values of the full posterior.  The code permits the user to specify the prior, though the reasoning behind the default is unclear.
...
Two simulated datasets were tested to validate the method, one representing an unrealistic toy model for $N(z)$ in Sec. \ref{sec:fake} and one based on a physically motivated $N(z)$ in Sec. \ref{sec:realnz}.%, and one based on physically motivated zPDFs in Sec. \ref{sec:realpz}.
...
%The number of galaxies in the survey $J$ is taken as a random draw from a Poisson distribution centered at $J'$.  
