...
products, they are most often employed in estimation of the redshift 
...
statistics.  This work challenges that paradigm, outlining instead a 
mathematically consistent technique motivated by fundamental probability theory 
in which a probabilistic graphical model corresponds to the hierarchical 
Bayesian relation of photo-z interim posteriors to a full posterior 
distribution over relevant physical hyperparameters.  The publicly available 
code produced herein takes as input a survey of photo-z interim posteriors and 
the interim prior used to derive them, and it produces as output Monte 
Carlo-Markov Chain samples of the full posterior distribution of 
hyperparameters describing the one-point statistic of redshift given a survey 
of photometric data used to derive the photo-z interim posteriors.  The 
approach is applied to the one-point statistic of the redshift distribution 
function $N(z)$, validated on synthetic data and tested on BOSS DR10 data.  It 
is demonstrated that this method improves accuracy, both qualitatively and as 
quantified by the Kullback-Leibler Divergence, compared to popular alternatives 
(such as stacking and conversion of photo-z interim posteriors to point 
estimates).
\end{abstract}
