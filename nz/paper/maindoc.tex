...
photo-$z$ PDFs than those that could fall into a spectroscopic training set (or 
spectroscopically derived template library).  This effect may be stronger for 
high-redshift galaxies.  This set of tests aims to demonstrate the performance 
of the sampler in cases with less and more random noise (via the factor $g$) 
and with noise that increases with redshift.
...
A second way to produce multimodal or asymmetric PDFs is to sample the simple 
Gaussians of the fiducial case at the level of the posteriors.  Here $K$ 
...
\caption{The catastrophic outlier PDFs case shows the same quantities as Fig. 
\ref{fig:nullpzs} but for multiple likelihood components (top panel) and 
sampled PDFs (bottom panel).}
\label{fig:allpzs}
\end{figure}
...
interim posteriors are shown in the top panel of Fig. \ref{fig:datapzs}.  The 
brightest half of this pseudo-random sampling are selected for another test, 
with photo-$z$ interim posterior samples shown in the bottom panel of Fig. 
\ref{fig:datapzs}.
...
\textbf{I will fill in KLD values, etc. when it looks like the test cases will 
no longer change.}
...
\ref{sec:imprecision}.  In the case of less noisy data, the error bars were 
smaller than in the fiducial case.  In both cases, the value of the mean of the 
samples was virtually unaffected.  Fig. \ref{fig:sig1-comp} shows this result 
for the $g=1$ case.  The alternative methods are still negatively affected in 
the optimistically narrow likelihood case and the case in which the likelihoods 
broaden with increasing redshift.
...
estimating the true $N(z)$ (thin, black line) even in the case of optimistic 
photo-$z$ interim posteriors.  The marginalized MAP (thin, dashed line; KLD) is 
heavily biased by the systematics of the rigid binning but otherwise performs 
similarly to stacking (thick, dashed line; KLD) and the marginalized expected 
value (thin, dotted line; KLD), smoothing the features of the distribution.  
The mean of sampled values (thick, solid line; KLD) and marginalized MLE 
(thick, dotted line; KLD) exhibit some bias at the peaks of the distribution 
but overall perform far better than the alternatives.}
\label{fig:sig1-comp}
\end{figure}

\begin{figure}
\includegraphics[width=0.5\textwidth]{figs/vars/comps.pdf}
\caption{Even in the case of simulated intrinsic scatter increasing with 
redshift, the sampler successfully recovers the true $N(z)$ whereas the 
alternatives fail to do so.}
\label{fig:vars-comp}
\end{figure}
