...
\caption{Several examples of individual photo-z posteriors are shown in colors. 
 The solid lines indicate the true redshift and the dotted lines indicate the 
shifted redshift used as the center of the Gaussian distribution.}
\label{fig:nullpzs}
\end{figure}
...
analysis, and an arbitrary six additional sub-runs are executed before the 
sampler terminates.  
...
\ref{sec:interim}).  Two additional cases with real data described in Sec. 
\ref{sec:data} are also considered.
...
another.  The corresponding sample interim photo-z posteriors to those of Fig. 
\ref{fig:nullpzs} are given in Figs. \ref{fig:sig2pzs} and \ref{fig:sig4pzs}.  
...
the estimate of $N(z)$, while the marginalized MLE enhances peaks and troughs 
to bias the estimate.
...
be considered here by introducing one change in the steps of Sec. 
\ref{sec:mock}.  
...
posterior distribution and a comparison to alternative methods.  It can be seen 
that the lessened quality of the data does impact the accuracy of the mean of 
the samples as an estimator of the truth.  However, the alternatives do worse 
as measured by the KLD, with stacking smoothing the peaks and troughs and the 
marginalized MLE enhancing them.
...
Fig. \ref{fig:toy-samp} shows some samples from the posterior and Fig. 
...
Examples of parameter values sampled from the full posterior are shown in Fig. 
\ref{fig:intu-samp}.  A comparison of the sampler to other approaches is shown 
in Fig. \ref{fig:intu-comp}.  It can be seen that the mean of the samples has a 
lower KLD than the result of stacking.
...
The results of the inference are shown in Fig. \ref{fig:intb-samp} shows 
examples of sampled parameter values from the full posterior and Fig. 
\ref{fig:intb-comp} shows the comparison of the mean of the sample values to 
the true values and the result of stacking.  What is most striking is that 
...
is not affected by it very much at all.
...
Based on these results, it can be concluded that when the interim prior is 
known to be inappropriate, the marginalized MLE is the best estimator of the 
truth.
...
sampling still performs better than stacking.  
