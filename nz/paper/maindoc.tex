...
\begin{figure}
\includegraphics[width=\textwidth]{figs/sig5/zobsvztru.pdf}
\caption{Top left panel: This plot depicts $p(z'_{j}|z^{*}_{j})$ for a randomly 
...
produce the horizontal slices shown in the right panel.  Right panel: In colors 
are plotted several examples of individual photo-$z$ posteriors (continuous in 
full opacity and in a binned parametrization in half opacity) in the fiducial 
case with their MLE redshifts (dashed lines) and true redshifts (solid lines); 
these represent horizontal slices through the top left panel.  Bottom panel: 
This plot shows the integral along horizontal slices through the top left 
panel, demonstrating that the individual photo-$z$ posteriors are properly 
normalized.}
\label{fig:nullpzs}
\end{figure}
