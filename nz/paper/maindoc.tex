...
equal the variable defining the Poisson distribution but instead $J$ by Eq. 
\ref{eq:definition}, which can be thought of as another parameter.  A detailed 
discussion of this matter may be found in \citet{ForemanMackey2014}.  Applying 
Bayes' Rule, we may combine terms to obtain 
\begin{equation}
\label{eq:posterior}
...
likelihood of Eq. \ref{eq:posterior} as 
\begin{equation}
\label{eq:trick}
...
\vec{\mu}=\vec{\theta}^{0}
\end{equation}
equal to the interim prior and covariance
\begin{equation}
\label{eq:priorcov}
\Sigma_{a,b} = q\ \exp[-\frac{e}{2}\ (\bar{z}_{a}-\bar{z}_{b})^{2}]\ +\ 
t\delta(a,b)
\end{equation}
inspired by one used in Gaussian processes.  This choice is made to permit 
draws from this prior distribution to produce shapes similar to that of the 
true $\tilde{\theta}$.  Here, the parameters of this covariance matrix are set 
to the small numbers $q=1.0$, $e=100.0$, and $t=q\cdot10^{-5}$.  We adapt the 
full log-posterior of Eq. \ref{eq:final} to the binning of redshift space 
chosen in Sec. \ref{sec:mock}.
