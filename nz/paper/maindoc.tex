...
Two simulated datasets were tested to validate the method, one representing an unrealistic toy model for $N(z)$ in Sec. \ref{sec:fake} and one based on a physically motivated $N(z)$ in Sec. \ref{sec:real}.  
...
This study demonstrates a mathematically consistent implementation of parameter inference of a one-point statistic based on zPDFs.  The method herein developed is applicable with minimal modification to other one-point statistics such as the weak lensing mean distance ratio.
