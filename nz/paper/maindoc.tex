...
\title{How to obtain one-point statistics of redshift from a survey of 
photometric redshift probability density functions}
...
\begin{abstract}
Since photometric redshift (photo-$z$) errors have become a limiting factor in 
constraining cosmological parameters from galaxy surveys, photo-$z$ point 
estimates are becoming subdominant to probability distribution functions (PDFs) 
as data products of photometric galaxy surveys.  In this paper, a fully 
probabilistic approach is developed to obtain MCMC samples of the full 
posterior distribution on a one-point statistic of redshift from a survey of 
photo-$z$ interim posteriors and the interim prior used to derive them.  The 
resulting technique uses a probabilistic graphical model corresponding to the 
hierarchical Bayesian relation of photo-$z$ PDFs to a posterior distribution 
over arbitrary redshift-dependent physical hyperparameters.  This 
...
weak lensing surveys, validated on synthetic data, and tested on BOSS DR10 
...
popular alternatives (such as stacking and conversion of photo-$z$ PDFs to 
point estimates).
\end{abstract}
...
remains to be done.  Of course, this brief review does not cover all ways to 
obtain photo-$z$ probability distribution functions; many more may be found in 
the literature, along with comparisons thereof \citep{Hildebrandt2010, 
...
which to demonstrate such an approach.  $N(z)$ is necessary for calculations of 
two-point correlation functions of weak gravitational lensing and counting 
...
Asorey2016}, so comparisons to the literature may be easily made.\textbf{Also 
discuss \citet{Leistedt2016} here for contrast.}
