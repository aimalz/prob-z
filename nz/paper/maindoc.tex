...
The code was tested on several simulated datasets with differing underlying $N(z)$ and the form of the zPDFs.  A preliminary test was run on the "toy model" case of a true redshift distribution that is a delta function, wherein all galaxies share the same true redshift.  
...
%- The "highly informative" case of the narrow true distribution (the case we did first).  All the plots again.
...
Two simulated datasets were tested to validate the method, one representing an unrealistic toy model for $N(z)$ in Sec. \ref{sec:fake} and a fiducial experiment based on a physically motivated $N(z)$ in Sec. \ref{sec:realnz}.%, and one based on physically motivated zPDFs in Sec. \ref{sec:realpz}.
