...
%\caption{Several examples of noisified individual interim photo-z posterior dis
...
%hs of the Gaussians vary randomly over the samples and do not necessarily corre
%late with redshift.}
%\label{fig:noisypzs}
%\end{figure}
%
...
In the second test case, catastrophic errors are simulated by considering 
multimodal photo-z likelihoods.  To achieve this goal we introduce another 
change in Sec. \ref{sec:mock}.  Here, the redshift posteriors are sums of 
Gaussians of the form of those tested in Sec. \ref{sec:null}.  Each galaxy is 
assigned a number $R_{j}$ of Gaussian elements to be summed, chosen randomly 
from $1,\dots,K$ with weights proportional to $k^{-1}$.  One $\sigma_{jr}$ is 
drawn for each $r=1,\dots,R_{j}$, and one $z'_{jr}$ is selected from the 
corresponding $\mathcal{N}(z^{0}_{j},\sigma^{2}_{jr})$.  The components are 
summed and normalized to yield the likelihood, which is then convolved with the 
interim prior to produce the interim posterior.  Some examples of the 
multimodal interim photo-z posteriors are shown in Fig. \ref{fig:multipzs}.  

\textbf{I redid this test with a few different schemes for attempting to make 
multimodal likelihoods but none worked well.  The one included here was 
especially bad because the sum of Gaussians with centers that are Gaussian 
distributed around the true redshift is guaranteed to lead to a MAP and 
expected value closer to the truth than the average individual Gaussian 
component.  I need to think of a good way to violate the generative model (as 
in the previous case in this section) that really gives likelihoods that look 
like the real data.}
...
\caption{\textbf{I don't know why the mean of samples is garbled like this when 
the sampled values look fine because it's being handled identically to the next 
case, which works without issue.  I'm looking into the problem, but 
unfortunately it's the slowest test case to run.}}
\label{fig:dataparam}
\end{figure}

\begin{figure}
\includegraphics[width=0.5\textwidth]{figs/boss/comps.pdf}
\caption{\textbf{Yes, I know, it happens on this one, too.  At least it's 
consistent.}}
\label{fig:datacomp}
\end{figure}
...
wrestling with Mendeley and BibDesk.}
